...
function onWindowResize(event) {
	...
	pathTracingRenderTarget.setSize(
		context.drawingBufferWidth,
		context.drawingBufferHeight
	);
	screenCopyRenderTarget.setSize(
		context.drawingBufferWidth,
		context.drawingBufferHeight
	);
	...
	pathTracingUniforms.uULen.value =
		pathTracingUniforms.uVLen.value * worldCamera.aspect;
...
function init() {
	...
	if (!mouseControl) isPaused = false;
...
function initTHREEjs() {
	...
	blueNoiseTexture = new THREE.TextureLoader().load(
	...
	fileLoader.load('shaders/ScreenCopy_Fragment.glsl', function (shaderText) {
		...
		//这里获取到当前渲染的结果
		screenCopyMaterial.uniforms.tPathTracedImageTexture.value =
	...
	fileLoader.load(
		...
		function (shaderText) {
			...
			screenOutputMaterial.uniforms.tPathTracedImageTexture.value =
				pathTracingRenderTarget.texture;

			screenOutputMesh = new THREE.Mesh(
				screenOutputGeometry,
...
function animate() {
	...
	if (!mouseControl) {
		...
		cameraControlsPitchObject.rotation.x = Math.max(
			-PI_2,
			Math.min(PI_2, cameraControlsPitchObject.rotation.x)
		);
	...
	if (useGenericInput) {
		if (!isPaused) {
			if (
				(keyPressed('w') || button3Pressed) &&
				!(keyPressed('s') || button4Pressed)
			) {
				cameraControlsObject.position.add(
					cameraDirectionVector.multiplyScalar(
						camFlightSpeed * frameTime
					)
				);
				cameraIsMoving = true;
			}
			...
			if (
				(keyPressed('a') || button1Pressed) &&
				!(keyPressed('d') || button2Pressed)
			) {
				cameraControlsObject.position.sub(
					cameraRightVector.multiplyScalar(camFlightSpeed * frameTime)
				);
				cameraIsMoving = true;
			}
			if (
			...
			if (keyPressed('q') && !keyPressed('z')) {
				cameraControlsObject.position.add(
					cameraUpVector.multiplyScalar(camFlightSpeed * frameTime)
				);
				cameraIsMoving = true;
			}
			...
			if (
				(keyPressed('up') || button5Pressed) &&
				!(keyPressed('down') || button6Pressed)
			) {
		...
		if (increaseFOV) {
			worldCamera.fov++;
			if (worldCamera.fov > 150) worldCamera.fov = 150;
			fovScale = worldCamera.fov * 0.5 * (Math.PI / 180.0);
			pathTracingUniforms.uVLen.value = Math.tan(fovScale);
			pathTracingUniforms.uULen.value =
				pathTracingUniforms.uVLen.value * worldCamera.aspect;
		...
		if (decreaseFOV) {
			worldCamera.fov--;
			if (worldCamera.fov < 1) worldCamera.fov = 1;
			fovScale = worldCamera.fov * 0.5 * (Math.PI / 180.0);
			pathTracingUniforms.uVLen.value = Math.tan(fovScale);
			pathTracingUniforms.uULen.value =
				pathTracingUniforms.uVLen.value * worldCamera.aspect;
		...
		if (decreaseFocusDist) {
			focusDistance -= 1;
			if (focusDistance < 1) focusDistance = 1;
			pathTracingUniforms.uFocusDistance.value = focusDistance;
			cameraIsMoving = true;
		...
		if (increaseAperture) {
			apertureSize += 0.1;
			if (apertureSize > 100.0) apertureSize = 100.0;
		...
		if (decreaseAperture) {
			apertureSize -= 0.1;
			if (apertureSize < 0.0) apertureSize = 0.0;
			pathTracingUniforms.uApertureSize.value = apertureSize;
			cameraIsMoving = true;
	...
	if (!cameraIsMoving) {
		if (sceneIsDynamic) sampleCounter = 1.0;
		// reset for continuous updating of image
	...
	//这里设置RTT，将path Tracing的渲染结果保存在这里
