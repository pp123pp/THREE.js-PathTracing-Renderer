...
function BVH_Create_Node(workList, idParent, isRightBranch) {
  ...
  if (workList.length < 1) {
    ...
    if (isRightBranch) buildnodes[idParent].idRightChild = flatLeafNode.idSelf;
    ...
    // if this is a right branch, fill in parent's missing link to this right child,
    // now that we have assigned this right child an ID
    if (isRightBranch) buildnodes[idParent].idRightChild = flatnode0.idSelf;
    ...
    minCentroid.set(Infinity, Infinity, Infinity); //这里重置整个节点的最小值，最大值和中心点
    maxCentroid.set(-Infinity, -Infinity, -Infinity);
    centroidAverage.set(0, 0, 0); //这里重置整个节点的中心点(质心)
    ...
    centroidAverage.divideScalar(workList.length); //计算质心
    ...
    // create inner node
    let flatnode = new BVH_FlatNode(); //创建节点
    ...
    // if this is a right branch, fill in parent's missing link to this right child,
    // now that we have assigned this right child an ID
    if (isRightBranch) buildnodes[idParent].idRightChild = flatnode.idSelf;
    ...
    // Try all 3 axes X, Y, Z //XYZ三个方向分割
    for (let axis = 0; axis < 3; axis++) {
      ...
      for (let partition = 1; partition < numBins; partition++) {
        ...
        //记录当前找到的最优分割,只有当计算的总代价<最小总代价时，当前节点才值得被分割
