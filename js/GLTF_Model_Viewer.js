...
function init_GUI() {
  ...
  sun_ColorController = gui
    .addColor(sun_ColorObject, "sunColor")
    .onChange(() => {
      ...
      meshGroup.traverse(function (child) {
        if (child.isMesh) {
          ...
          child.geometry.applyMatrix4(
            child.matrix.multiply(matrixStack[matrixStack.length - 1])
          );
...
function prepareGeometryForPT(
  ...
  let total_number_of_triangles =
    modelMesh.geometry.attributes.position.array.length / 9;
  ...
  for (let i = 0; i < meshList.length; i++) {
    if (meshList[i].material.length > 0) {
      for (let j = 0; j < meshList[i].material.length; j++) {
        if (meshList[i].material[j].map) {
          for (let k = 0; k < uniqueMaterialTextures.length; k++) {
            if (
              meshList[i].material[j].map.image.src ===
              uniqueMaterialTextures[k].image.src
            ) {
      ...
      for (let j = 0; j < uniqueMaterialTextures.length; j++) {
        if (
          meshList[i].material.map.image.src ===
          uniqueMaterialTextures[j].image.src
        ) {
  ...
  for (let i = 0; i < total_number_of_triangles; i++) {
    ...
    triangle_array[32 * i + 25] =
      pathTracingMaterialList[materialNumber].color.r; // g or y
    triangle_array[32 * i + 26] =
      pathTracingMaterialList[materialNumber].color.g; // b or z
    triangle_array[32 * i + 27] =
      pathTracingMaterialList[materialNumber].color.b; // a or w

    //slot 7
    triangle_array[32 * i + 28] =
      pathTracingMaterialList[materialNumber].albedoTextureID; // r or x
    triangle_array[32 * i + 29] =
      pathTracingMaterialList[materialNumber].opacity; // g or y
    ...
    triangle_b_box_centroid
      .copy(triangle_b_box_min)
      .add(triangle_b_box_max)
      .multiplyScalar(0.5);
...
function initSceneData() {
  ...
  pathTracingUniforms.uSunColor = {
    value: new THREE.Color().fromArray(sunColor.map((x) => x)),
  };
...
function updateVariablesAndUniforms() {
  ...
  if (skyLightIntensityChanged) {
    pathTracingUniforms.uSkyLightIntensity.value =
      skyLight_IntensityController.getValue();
    cameraIsMoving = true;
  ...
  if (sunAngleChanged) {
    ...
    sunDirection.set(
      Math.cos(sunAngle) * 1.2,
      Math.sin(sunAngle),
      -Math.cos(sunAngle) * 3.0
    );
  ...
  if (sunLightIntensityChanged) {
    pathTracingUniforms.uSunLightIntensity.value =
      sunLight_IntensityController.getValue();
    cameraIsMoving = true;
  ...
  if (sunColorChanged) {
    ...
    pathTracingUniforms.uSunColor.value.setRGB(
      sunColor[0],
      sunColor[1],
      sunColor[2]
    );
